...
/* Returns TRUE on success ans FALSE on any error							*/
{
	...
	printf("PID: %i , FRAME: %i\n", processTable[pid].pid, frame);
...
/* Returns TRUE on success and FALSE on any error							*/
{
	...
	*/
	...
	for (int i = 1; i <= MEMORYSIZE; i++) {
		for(paget = 0; paget < processTable[i].size; paget++){
			if (processTable[i].pageTable[paget].referenced == TRUE) {
				memories[i - 1].referenced = 1;
			}
			if (processTable[i].pageTable[paget].modified == TRUE) {
				memories[i - 1].modified = 1;
			}
		}
		if (memories[i - 1].referenced != 1) memories[i - 1].referenced = 0;
		if (memories[i - 1].modified != 1) memories[i - 1].modified = 0;
	}

	for (int i = 0; i < MEMORYSIZE; i++) {
		printf("%d: [ %d , %d ]\n", i, memories[i].referenced, memories[i].modified);
	}
	...
	for (int i = 0; i < 4 && replacementFound == FALSE; i++) {
		if (i == 0) {
			...
			for (int f = 0; f < MEMORYSIZE; f++) {
				if (memories[f].referenced == 0 && memories[f].modified == 0) {
					frame = f;
		...
		if (i == 1) {
			printf("	-> Level 1: \n");
			for (int f = 0; f < MEMORYSIZE; f++) {
				if (memories[f].referenced == 0 && memories[f].modified == 1) {
					frame = f;
					replacementFound = TRUE;
					break;
				}
		...
		if (i == 2) {
			printf("	-> Level 2: \n");
			for (int f = 0; f < MEMORYSIZE; f++) {
				if (memories[f].referenced == 1 && memories[f].modified == 0) {
					frame = f;
					replacementFound = TRUE;
					break;
				}
		...
		if (i == 3) {
			printf("	-> Level 3: \n");
			for (int f = 0; f < MEMORYSIZE; f++) {
				if (memories[f].referenced == 1 && memories[f].modified == 1) {
					frame = f;
					replacementFound = TRUE;
					break;
				}
